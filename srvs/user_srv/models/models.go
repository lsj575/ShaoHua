...
type User struct {
	...
	Score            uint64         `gorm:"not null;index:idx_user_score" json:"score" form:"score"`            // 积分
	Status           int32          `gorm:"index:idx_user_status;not null" json:"status" form:"status"`         // 状态
	ArticleCount     uint64         `gorm:"not null" json:"topicCount" form:"topicCount"`                       // 帖子数量
	CommentCount     uint64         `gorm:"not null" json:"commentCount" form:"commentCount"`                   // 评论数量
	Roles            int32          `gorm:"type:text" json:"roles" form:"roles"`                                // 角色
	Type             int32          `gorm:"not null" json:"type" form:"type"`                                   // 用户类型
	Gender           uint32         `gorm:"not null;default:0" json:"gender" form:"gender"`                     // 用户性别
	ForbiddenEndTime uint64         `gorm:"not null;default:0" json:"forbiddenEndTime" form:"forbiddenEndTime"` // 禁言结束时间
